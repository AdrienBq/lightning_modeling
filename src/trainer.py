...
class Trainer(ABC):
    def __init__(self,
        ...
        self.model_name = self.model.name if hasattr(self.model, 'name') else 'model_name_not_specified'
    ...
    def train(self, **kwargs) -> torch.Tensor:
        ...
        print(f'Training model with {n_params} parameters (size: {size_b / MiB:.3f} MiB)')
        ...
        for idx, epoch in enumerate(range(self.num_epochs)):
            ...
            if kwargs.get('intermediate_save', False):
                ...
                torch.save(self.model.state_dict(), self.model.save_path / f'{self.model.name}_epoch_{idx}.pth')
                # remove the previous model if the file exists
                previous_model_path = self.model.save_path / f'{self.model.name}_epoch_{idx-1}.pth'
        ...
        torch.save(self.model.state_dict(), self.model.save_path / f'{self.model.name}.pth')
        losses_pd.to_csv(self.model.save_path / f'{self.model.name}_losses.csv')
        self.model.eval()
    ...
    def recalibrate(self):
        if hasattr(self.model, 'recalibration_method'):
            ...
            print('No valid recalibration method specified in the model. Defaulting to no recalibration.')
    ...
    def platt_recalibration(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.calibration_dataloader):
                if self.calibration_early_stopping and i >= n_batches:   # use only 200 batches for recalibration
                    break
                if i%100 == 0:
                    print(f"{i}/{n_batches}, recalibration duration : {time.time() - start_time}")
        ...
        self.model.recalibration_layer.a = nn.Parameter(torch.tensor(self.recalibration.coef_, dtype=torch.float32, device=self.device))
        self.model.recalibration_layer.b = nn.Parameter(torch.tensor(self.recalibration.intercept_, dtype=torch.float32, device=self.device))
        # save the model
        torch.save(self.model.state_dict(), self.model.save_path / f'{self.model.name}.pth')

        print(f'Recalibration finished ! Total time : {time.time() - start_time}')
        print(f'Platt recalibration coefficients: {self.recalibration.coef_}, intercept: {self.recalibration.intercept_}')
    ...
    def test(self, extremes=False) -> float:
        ...
        self.metrics = Metrics(kernel_size=1, save_path=self.eval_path, reduction='mean', n_batches=n_batches, extremes=extremes)
        ...
        with torch.no_grad():
            for i, batch in enumerate(loader):
                ...
                if i+1%100 == 0 :
                    print(f"{i}/{min(n_batches, len(loader))}, test duration : {time.time() - test_start_time}")
                batch = batch.view(bs * T, C, H, W)     # shape (bs * T, c+2, h, w)
                x = batch[:, :-1, :, :].to(self.device)
